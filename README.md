...
bl2_ext 内部**可能**有独立的签名验证逻辑（RSA+SHA，不走 CERT2），会二次验证 LK/ATF。

**是否需要 patch bl2_ext？**

- **不一定。** 部分 V6 设备的 bl2_ext 验证在 SBC（Secure Boot Control）efuse 未烧写时是关闭的，或者验证策略允许跳过。这种情况下只做 cert bypass 签名 bl2_ext 本身即可，不需要 patch 代码。
- **需要 patch 的情况：** bl2_ext 内部强制验证 LK/ATF 且 cert bypass 无法绕过时（bl2_ext 不使用 CERT2 的 ASN.1 解析），才需要 patch 验证函数。
- **判断方法：** 先只做 cert bypass 不 patch，刷入测试。如果 LK/ATF 加载失败（bl2_ext 报签名错误），再用 `patch_bl2_ext.py` patch。
...
## 操作流程：V6 设备完整 bypass（有 bl2_ext）

> **注意：** 不是所有 V6 设备都需要 patch bl2_ext。先只做 cert bypass 签名测试，只有 bl2_ext 验证确实阻止了 LK/ATF 加载时才需要 patch。

```bash
# 直接对 lk.img 操作（自动定位 bl2_ext 子镜像）：
python3 patch_bl2_ext.py lk.img --minimal -o lk_patched.img

# 或拆分后单独 patch：
python3 parse-part-img.py lk.img --split -o lk_parts/
python3 patch_bl2_ext.py lk_parts/bl2_ext.bin --minimal -o lk_parts/bl2_ext_patched.bin

# 先 --dry-run 查看 patch 点，确认无误再写入：
python3 patch_bl2_ext.py lk.img --minimal --dry-run
```

patch 之后仍需 cert bypass 签名（让 preloader 接受修改后的 bl2_ext）：

```bash
# 如果直接 patch 了 lk.img，对 bl2_ext 子镜像做 cert bypass
